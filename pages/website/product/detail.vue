	...
	<view class="page">
		...
		<template v-if="loaded">
			...
			<view class="cu-bar bg-white tabbar shop foot">
				...
				<button open-type="contact" class="bg-red submit" style="border-radius: 0;">立即咨询</button>
			</view>
		</template>
		...
		methods: {
			...
			toggleFavorite() {
				...
				if (this.info.is_favorite) {
					uni.$http.get('/plugin/website/product/unfavorite', {
					...
					uni.$http.get('/plugin/website/product/favorite', {
						topic_id: this.info.id
					}, options).then(() => {
			...
			onLike() {
				...
				uni.$http.get('/plugin/website/product/like', {
					topic_id: this.info.id
				}, {
					hint: this,
					loading: this
