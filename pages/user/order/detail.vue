			...
			<view class="bg-white margin-top">
				...
				<view class="cu-list goods-list">
					<view class="cu-item flex padding-sm" v-for="(goodsItem,goodsIndex) in info.goods_list"
						...
						<view class="action">
							<view class="text-price text-red text-lg text-bold text-right">{{ goodsItem.goods_price }}
							</view>
...
<script>
	export default {
		...
		methods: {
			// 加载数据
			loadData() {
				return uni.$models.order.getOrderDetail(this.id).then(res => {
					this.info = res;
					this.loaded = true;
				});
			},
			...
			deleteOrder() {
				uni.showModal({
					...
					success: (res) => {
						...
						uni.$models.order.deleteOrder(this.id, {
							loading: this,
							hint: this,
						}).then((res) => {
			...
			cancelOrder(index) {
				uni.showModal({
					...
					success: (res) => {
						...
						uni.$models.order.setOrderCancel(this.id, {
							loading: this,
							hint: this,
						}).then((res) => {
